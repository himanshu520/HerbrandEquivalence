...
% \begin{frame}{Herbrand Equivalence}
% Suppose our expressions in the program are defined by the following grammar
% $$ e\ ::=\ c\ |\ x\ |\ (e_1 + e_2)$$
% where c and x are constant and variables occurring in the program respectively
% \begin{itemize}
%     \item $c$ is Herbrand equivalent to only itself
%     \item $x$ is Herbrand equivalent to $e$ at a program point, if assignment $x := e$ is \textit{reaching} at the current program point
%     \item $e_1 + e_2$ is Herbrand equivalent to $e_3 + e_4$ at a program point iff $e_1$ and $e_2$ are Herbrand equivalent to $e_3$ and $e_4$ respectively at the current program point
% \end{itemize}
% We can extend this definition to more than one operator in which case the operator must also be the same in the last case above
% \end{frame}

\begin{frame}{Example of Herbrand Equivalence Computation}
    \begin{figure}[!h]
        \centering
        \includegraphics[scale=0.3]{HerbrandEquivalence.png}
...
\begin{frame}{Example of Herbrand Equivalence Computation}
    \begin{figure}[!h]
        \centering
        \includegraphics[scale=0.4]{HerbrandEquivalenceCon.png}
